...
export const getCourseProgress = cache(async () => {
  ...
  const firstUncompletedLesson = unitsInAcitveCourse
  ...
  return {
    ...
    activeLessId: firstUncompletedLesson?.id,
  };
});
...
export const getLesson = cache(async (id?: number) => {
  ...
  const lessonId = id || courseProgress?.activeLessId;
  if (!lessonId) {
    return null;
  }
...
export const getLessonPercentage = cache(async () => {
  const courseProgress = await getCourseProgress();
  if (!courseProgress?.activeLessId) return 0;

  const lesson = await getLesson(courseProgress.activeLessId);
  if (!lesson) return 0;
